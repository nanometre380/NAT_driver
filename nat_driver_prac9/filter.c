...
void checksumadjust(UCHAR *chksum, UCHAR *optr, int olen, UCHAR *nptr, int nlen)
{
	long x, old, new;
	...
	x = ~x;
	chksum[0] = (UCHAR)(x / 256); chksum[1] = (UCHAR)(x & 0xff);
}
...
--*/
{
    ...
    do
    {
        ...
        Status = natdriverprac9RegisterDevice();
...
--*/
{
    ...
    DEBUGP(DL_TRACE, "===>natdriverprac9 FilterPause: FilterInstance %p\n", FilterModuleContext);
...
--*/
{
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        ...
        //
        PWCHAR              ErrorString = L"natdriverprac9";
...
--*/
{
    ...
    //
    natdriverprac9DeregisterDevice();
    NdisFDeregisterFilterDriver(FilterDriverHandle);
...
--*/
{
	...
	PNET_BUFFER_LIST IterNbl;

	DEBUGP(DL_TRACE, "\n=====NBL address test in CMPLT=====\n");
	DEBUGP(DL_TRACE, "CurrNbl address : %p \n", CurrNbl);
	DEBUGP(DL_TRACE, "ParentNbl address : %p \n", CurrNbl->ParentNetBufferList);

	IterNbl = NetBufferLists;
	NextNbl = NULL;
	while (IterNbl) {

		NextNbl = NET_BUFFER_LIST_NEXT_NBL(IterNbl);

		NetBuffer = NET_BUFFER_LIST_FIRST_NB(IterNbl);

		eth = (struct ethernet_h *) NdisGetDataBuffer(NetBuffer, 14, NULL, 1, 0);

		if (eth != NULL) {
			DEBUGP(DL_TRACE, "type : %4x \n", RtlUshortByteSwap(eth->type));

			if (RtlUshortByteSwap(eth->type) == 0x800) {

				DEBUGP(DL_TRACE, "eth type : ip 0x800 \n");
				NdisAdvanceNetBufferDataStart(NetBuffer, 14, FALSE, NULL);

				ip = (struct ip_h *)NdisGetDataBuffer(NetBuffer, 20, NULL, 1, 0);

				if (ip != NULL) {
					size_iphl = (ip->ip_v_l & 0x0f) * sizeof(ULONG);

					DEBUGP(DL_TRACE, "\n ====IP headertest : \n ");
					DEBUGP(DL_TRACE, "Protocol : %d\n", ip->ip_protocol);
					DEBUGP(DL_TRACE, "IP src : %d.%d.%d.%d \n", ip->ip_src.S_un.S_un_b.s_b1, ip->ip_src.S_un.S_un_b.s_b2, ip->ip_src.S_un.S_un_b.s_b3, ip->ip_src.S_un.S_un_b.s_b4);
					DEBUGP(DL_TRACE, "IPchecksum : %x \n", ip->chksum);
					DEBUGP(DL_TRACE, "SIZE_IPHL = %d \n", (int)size_iphl);

					NdisAdvanceNetBufferDataStart(NetBuffer, (ULONG)size_iphl, FALSE, NULL);

					if (ip->ip_protocol == 6) {
						tcp = (struct tcp_h*)NdisGetDataBuffer(NetBuffer, 20, NULL, 1, 0);
						if (tcp != NULL) DEBUGP(DL_TRACE, "\n ==== TCP checksum : %x \n", RtlUshortByteSwap(tcp->tcp_checksum));
					}
					NdisRetreatNetBufferDataStart(NetBuffer, (ULONG)size_iphl, FALSE, NULL);

				}
				NdisRetreatNetBufferDataStart(NetBuffer, 14, FALSE, NULL);
			}
		}
		...
		if (IterNbl->ParentNetBufferList != NULL) {
			if (IterNbl->SourceHandle == FilterModuleContext) {
				ParentNbl = IterNbl->ParentNetBufferList;
				if (ParentNbl) {
					NET_BUFFER_LIST_STATUS(ParentNbl) = NET_BUFFER_LIST_STATUS(IterNbl);
					ParentNbl->ChildRefCount--;
					NET_BUFFER_LIST_NEXT_NBL(ParentNbl) = NET_BUFFER_LIST_NEXT_NBL(IterNbl);
				...
				IterNbl->ParentNetBufferList = NULL;
				NdisFreeCloneNetBufferList(IterNbl, 0);
			}
			else {
				PrevNbl = IterNbl;
			}
...
--*/
{
    ...
    do
    {
       ...
       DispatchLevel = NDIS_TEST_SEND_AT_DISPATCH_LEVEL(SendFlags);
		...
		PNET_BUFFER_LIST	IterNbl = NetBufferLists;
		PNET_BUFFER_LIST	PrevNbl = NULL;
		PNET_BUFFER_LIST	FirstNbl = NULL;
		//PNET_BUFFER_LIST	ItCurrNbl = NULL;


		struct ethernet_h *eth;			//eth header
		struct ip_h *ip = NULL;			//ip header
		struct tcp_h *tcp = NULL;		//tcp header

		int size_iphl = 0;				//ip header length

		UCHAR		buffer_eth[14], buffer_ip[20]; //eth header size, ip header size

		int IsClone = 0; //whether clonenbl created
		...
		while (IterNbl != NULL) {
			...
			IsClone = 0;
			CurrNbl = IterNbl;
			IterNbl = NET_BUFFER_LIST_NEXT_NBL(IterNbl);
			//CurrNbl->Next = NULL;

			if (PrevNbl != NULL) PrevNbl->Next = CurrNbl;
			else FirstNbl = CurrNbl;

			eth = (struct ethernet_h *)NdisGetDataBuffer(NetBuffer, sizeof(buffer_eth), NULL, 1, 0);	//get ethernet header

			if (eth != NULL) {
				DEBUGP(DL_TRACE, "\n====Ethernet headertest : \n MAC src : %04x:%04x:%04x \n", eth->src_mac.byte1, eth->src_mac.byte2, eth->src_mac.byte3);
				DEBUGP(DL_TRACE, "MAC dst : %04x:%04x:%04x \n", eth->dst_mac.byte1, eth->dst_mac.byte2, eth->dst_mac.byte3);
				DEBUGP(DL_TRACE, "type : %4x \n", RtlUshortByteSwap(eth->type));

				if (RtlUshortByteSwap(eth->type) == 0x800) {										//ethertype이 IPv4(0x0800)이면

					DEBUGP(DL_TRACE, "eth type : ip 0x800 \n");

					NdisAdvanceNetBufferDataStart(NetBuffer, sizeof(buffer_eth), FALSE, NULL);		//eth header만큼 제거

					ip = (struct ip_h *)NdisGetDataBuffer(NetBuffer, sizeof(buffer_ip), NULL, 1, 0);//get ip headeer

					if (ip != NULL) {
						size_iphl = (ip->ip_v_l & 0x0f) * sizeof(ULONG);
						char ip_temp[20];
						sprintf(ip_temp, "%d.%d.%d.%d", ip->ip_dst.S_un.S_un_b.s_b1, ip->ip_dst.S_un.S_un_b.s_b2, ip->ip_dst.S_un.S_un_b.s_b3, ip->ip_dst.S_un.S_un_b.s_b4);//get ip header length

						DEBUGP(DL_TRACE, "\n ====IP headertest : \n ");
						DEBUGP(DL_TRACE, "Protocol : %d\n", ip->ip_protocol);
						DEBUGP(DL_TRACE, "Header Length : %d\n", (int)(ip->ip_v_l & 0x0f));
						DEBUGP(DL_TRACE, "IP src : %d.%d.%d.%d \n", ip->ip_src.S_un.S_un_b.s_b1, ip->ip_src.S_un.S_un_b.s_b2, ip->ip_src.S_un.S_un_b.s_b3, ip->ip_src.S_un.S_un_b.s_b4);
						DEBUGP(DL_TRACE, "IP dst : %d.%d.%d.%d \n", ip->ip_dst.S_un.S_un_b.s_b1, ip->ip_dst.S_un.S_un_b.s_b2, ip->ip_dst.S_un.S_un_b.s_b3, ip->ip_dst.S_un.S_un_b.s_b4);
						DEBUGP(DL_TRACE, "Ip checksum : %x \n", ip->chksum);
						DEBUGP(DL_TRACE, "SIZE_IPHL = %d \n", (int)size_iphl);



						NdisAdvanceNetBufferDataStart(NetBuffer, (ULONG)size_iphl, FALSE, NULL);	//ip header 길이만큼 제거

						if (ip->ip_protocol == 6) {													//ip protocol이 6 (tcp)인 경우
							tcp = (struct tcp_h*)NdisGetDataBuffer(NetBuffer, 20, NULL, 1, 0);		//get tcp header

							if (tcp != NULL) {
								DEBUGP(DL_TRACE, "\n ====TCP headertest : \n ");
								DEBUGP(DL_TRACE, "src port : %d     dst port : %d \n", (int)RtlUshortByteSwap(tcp->src_port), (int)RtlUshortByteSwap(tcp->dest_port));
								DEBUGP(DL_TRACE, "tcp chksum : %d \n", (int)RtlUshortByteSwap(tcp->tcp_checksum));

							}

							NdisAdvanceNetBufferDataStart(NetBuffer, 20, FALSE, NULL);				//tcp header 길이만큼 제거

							if (strcmp(ip_temp, "10.20.20.50") == 0 && (int)RtlUshortByteSwap(tcp->dest_port) == 22) {

								// 목적지 주소가 broadcast가 아니고, 외부 주소이고, 출발지 주소가 내부 주소이면!
								DEBUGP(DL_TRACE, "HAVE TO CHANGE\n");

								ClonedNbl = NdisAllocateCloneNetBufferList(CurrNbl, NULL, NULL, 0);		//header가 모두 제거된 Nbl clone

								if (ClonedNbl != NULL) {
									if (PrevNbl != NULL) PrevNbl->Next = ClonedNbl;
									else FirstNbl = ClonedNbl;
									IsClone = 1;
								}								//whether clone created
								else {
									IsClone = 0;
								}

								NdisRetreatNetBufferListDataStart(ClonedNbl, sizeof(buffer_eth) + (int)size_iphl + 20, FALSE, NULL, NULL); // clonednbl에 헤더 공간 할당
							}

							NdisRetreatNetBufferDataStart(NetBuffer, 20, FALSE, NULL);				//tcp header 복구

						}

						NdisRetreatNetBufferDataStart(NetBuffer, (ULONG)size_iphl, FALSE, NULL);	//ip header 복구
					}

					NdisRetreatNetBufferDataStart(NetBuffer, sizeof(buffer_eth), FALSE, NULL);		//eth header 복구
			...
			if (IsClone == 1) {
				...
				struct ethernet_h *m_eth = NdisGetDataBuffer(ClonedNb, sizeof(buffer_eth), NULL, 1, 0);
				*m_eth = *eth;
				m_eth->dst_mac.byte1 = 0x0008;
				m_eth->dst_mac.byte2 = 0xee27;
				m_eth->dst_mac.byte3 = 0x8d05;

				NdisAdvanceNetBufferDataStart(ClonedNb, sizeof(buffer_eth), FALSE, NULL);

				struct ip_h *m_ip = NdisGetDataBuffer(ClonedNb, sizeof(buffer_ip), NULL, 1, 0);
				*m_ip = *ip;

				NdisAdvanceNetBufferDataStart(ClonedNb, sizeof(buffer_ip) - 10, FALSE, NULL);
				PUSHORT ip_checksum = NdisGetDataBuffer(ClonedNb, 2, NULL, 1, 0);

				NdisAdvanceNetBufferDataStart(ClonedNb, 6, FALSE, NULL);
				struct in_addr *m_dst = (struct in_addr *)NdisGetDataBuffer(ClonedNb, 4, NULL, 1, 0);
				...
				//PCSTR strip = "210.89.164.90";
				PCSTR strip = "10.20.20.40";

				//if (STATUS_SUCCESS == RtlIpv4StringToAddress(TEXT("210.89.164.90"), TRUE, &strip, &m_dst_ip)) {
				if (STATUS_SUCCESS == RtlIpv4StringToAddress(strip, TRUE, &strip, &m_dst_ip)) {

					*m_dst = m_dst_ip;

					DEBUGP(DL_TRACE, "=====print ip address after change===== : %d.%d.%d.%d \n", m_dst->S_un.S_un_b.s_b1, m_dst->S_un.S_un_b.s_b2, m_dst->S_un.S_un_b.s_b3, m_dst->S_un.S_un_b.s_b4);
				}
				else {
				...
				checksumadjust((UCHAR*)ip_checksum, (UCHAR*)&(ip->ip_dst.S_un.S_addr), sizeof(ULONG), (UCHAR*)&(m_ip->ip_dst.S_un.S_addr), sizeof(ULONG));
				*ip_checksum = *ip_checksum - 0x0101;

				if (ip->ip_protocol == 6) {
					NdisAdvanceNetBufferDataStart(ClonedNb, (int)size_iphl - 16, FALSE, NULL);
					struct tcp_h *m_tcp = (struct tcp_h *)NdisGetDataBuffer(ClonedNb, 20, NULL, 1, 0);
					*m_tcp = *tcp;

					NdisAdvanceNetBufferDataStart(ClonedNb, 16, FALSE, NULL);
					PUSHORT tcp_checksum = NdisGetDataBuffer(ClonedNb, 2, NULL, 1, 0);

					//*tcp_checksum = 0;

					checksumadjust((UCHAR*)tcp_checksum, (UCHAR*)&(ip->ip_dst.S_un.S_addr), sizeof(ULONG), (UCHAR*)&(m_ip->ip_dst.S_un.S_addr), sizeof(ULONG));
					*tcp_checksum = *tcp_checksum - 0x0101;


					NdisRetreatNetBufferDataStart(ClonedNb, 16, FALSE, NULL);
				}

				NdisRetreatNetBufferDataStart(ClonedNb, sizeof(buffer_eth) + (int)size_iphl, FALSE, NULL);

				//-------첫번째 MDL 확인
				DEBUGP(DL_TRACE, "\n====address test====\n");
				DEBUGP(DL_TRACE, "original nbl : %p \n", CurrNbl);
				DEBUGP(DL_TRACE, "cloned nbl : %p \n", ClonedNbl);
				DEBUGP(DL_TRACE, "First nbl : %p \n", FirstNbl);

				NDIS_TCP_IP_CHECKSUM_NET_BUFFER_LIST_INFO nblChecksumInfo;
				NDIS_TCP_IP_CHECKSUM_NET_BUFFER_LIST_INFO ChecksumInfo2;
				nblChecksumInfo.Value = NET_BUFFER_LIST_INFO(CurrNbl, TcpIpChecksumNetBufferListInfo);
				NET_BUFFER_LIST_INFO(ClonedNbl, TcpIpChecksumNetBufferListInfo) = nblChecksumInfo.Value;
				ChecksumInfo2.Value = NET_BUFFER_LIST_INFO(ClonedNbl, TcpIpChecksumNetBufferListInfo);

				DEBUGP(DL_TRACE, "\n===checksum===\n");
				DEBUGP(DL_TRACE, "IsIPv4 : %d, IpHeaderCheckSum : %d \n", ChecksumInfo2.Transmit.IsIPv4, ChecksumInfo2.Transmit.IpHeaderChecksum);
				DEBUGP(DL_TRACE, "TcpChecksum : %d,  UdpChecksum ; %d \n", ChecksumInfo2.Transmit.TcpChecksum, ChecksumInfo2.Transmit.UdpChecksum);
				DEBUGP(DL_TRACE, "TcpheaderOffset : %d \n", ChecksumInfo2.Transmit.TcpHeaderOffset);
				...
				//NdisSendNetBufferLists(pFilter->FilterHandle, ClonedNbl, PortNumber, SendFlags);

			}
			if (IterNbl != NULL) {
				if (IsClone == 1) PrevNbl = ClonedNbl;
				else PrevNbl = CurrNbl;
			}
		...
		//if (IsClone == 1) {
		//	NdisFSendNetBufferLists(pFilter->FilterHandle, FirstNbl, PortNumber, SendFlags);
		//}
		//else {
		//	NdisFSendNetBufferLists(pFilter->FilterHandle, FirstNbl, PortNumber, SendFlags);
		//}
		DEBUGP(DL_TRACE, "LAST TEST== \n FirstNBL : %p \n", FirstNbl);
		DEBUGP(DL_TRACE, "NetBufferLists : %p \n", NetBufferLists);
		NdisFSendNetBufferLists(pFilter->FilterHandle, FirstNbl, PortNumber, SendFlags);
...
--*/
{
    ...
    do
    {
		...
		DEBUGP(DL_TRACE, "~~~~~~~~~NUMBER OF NBL : %d ~~~~~~~~~\n", NumberOfNetBufferLists);
		...
		while (IterNbl != NULL) {
			...
			if (eth != NULL) {
				...
				if (RtlUshortByteSwap(eth->type) == 0x800) {
					...
					if (ip != NULL) {
						size_iphl = (ip->ip_v_l & 0x0f) * sizeof(ULONG);
						char ip_temp[20];
						sprintf(ip_temp, "%d.%d.%d.%d", ip->ip_src.S_un.S_un_b.s_b1, ip->ip_src.S_un.S_un_b.s_b2, ip->ip_src.S_un.S_un_b.s_b3, ip->ip_src.S_un.S_un_b.s_b4);//get source ip
						...
						DEBUGP(DL_TRACE, "Ip checksum : %x \n", ip->chksum);
						DEBUGP(DL_TRACE, "SIZE_IPHL = %d \n", (int)size_iphl);
						DEBUGP(DL_TRACE, "ip_temp : %s \n", ip_temp);

						if (ip->ip_protocol == 6) {
							NdisAdvanceNetBufferDataStart(NetBuffer, size_iphl, FALSE, NULL);
							tcp = (struct tcp_h *)NdisGetDataBuffer(NetBuffer, 20, NULL, 1, 0);

							if (tcp != NULL) {
								DEBUGP(DL_TRACE, "\n==== TCP header test : \n");
								DEBUGP(DL_TRACE, "TCP src port : %d \n", (int)RtlUshortByteSwap(tcp->src_port));

								NdisAdvanceNetBufferDataStart(NetBuffer, 20, FALSE, NULL);
								if ((int)RtlUshortByteSwap(tcp->src_port) == 22 && strcmp(ip_temp, "10.20.20.40") == 0) {

									DEBUGP(DL_TRACE, "HAVE TO CHANGE \n");
									
									ClonedNbl = NdisAllocateCloneNetBufferList(CurrNbl, NULL, NULL, 0);
									...
									if (ClonedNbl != NULL) {
										...
										NdisRetreatNetBufferListDataStart(ClonedNbl, 14 + (int)size_iphl + 20, FALSE, NULL, NULL);
									}
								}
								NdisRetreatNetBufferDataStart(NetBuffer, 20, FALSE, NULL);
							}
							NdisRetreatNetBufferDataStart(NetBuffer, (int)size_iphl, FALSE, NULL);
						}
			...
			if (IsClone == 1) {
				...
				struct ethernet_h *m_eth = NdisGetDataBuffer(ClonedNb, 14, NULL, 1, 0);
				*m_eth = *eth;
				m_eth->src_mac.byte1 = 0x0e04;
				m_eth->src_mac.byte2 = 0x6b3c;
				m_eth->src_mac.byte3 = 0x7f1a; //10.20.20.50의 MAC주소

				NdisAdvanceNetBufferDataStart(ClonedNb, 14, FALSE, NULL);

				struct ip_h *m_ip = NdisGetDataBuffer(ClonedNb, 14, NULL, 1, 0);
				*m_ip = *ip;

				NdisAdvanceNetBufferDataStart(ClonedNb, 20 - 10, FALSE, NULL);
				PUSHORT ip_checksum = NdisGetDataBuffer(ClonedNb, 2, NULL, 1, 0);
				*ip_checksum = 0;

				NdisAdvanceNetBufferDataStart(ClonedNb, 2, FALSE, NULL);
				struct in_addr *m_src = (struct in_addr *)NdisGetDataBuffer(ClonedNb, 4, NULL, 1, 0);

				struct in_addr m_src_ip = { 0 };

				PCSTR strip = "10.20.20.50";

				if (STATUS_SUCCESS == RtlIpv4StringToAddress(strip, TRUE, &strip, &m_src_ip)) {
					*m_src = m_src_ip;

					DEBUGP(DL_TRACE, "=====print ip address after change===== : %d.%d.%d.%d \n", m_src->S_un.S_un_b.s_b1, m_src->S_un.S_un_b.s_b2, m_src->S_un.S_un_b.s_b3, m_src->S_un.S_un_b.s_b4);
				}
				else {
					DEBUGP(DL_TRACE, "Failed RtlIpv4StringToAddress");
				}
				...
				NdisMoveMemory(ipdata, &psdip, 20);

				for (int i = 0; i < 10; i++) {
					DEBUGP(DL_TRACE, "%x  ", ipdata[i]);
				}
				DEBUGP(DL_TRACE, "\n");
				*ip_checksum = CheckSum((USHORT *)ipdata, sizeof(psdip));

				if (ip->ip_protocol == 6) {
					NdisAdvanceNetBufferDataStart(ClonedNb, (int)size_iphl - 12, FALSE, NULL);
					struct tcp_h *m_tcp = (struct tcp_h *)NdisGetDataBuffer(ClonedNb, 20, NULL, 1, 0);
					*m_tcp = *tcp;
					...
					checksumadjust((UCHAR*)tcp_checksum, (UCHAR*)&(ip->ip_src.S_un.S_addr), sizeof(ULONG), (UCHAR*)&(m_ip->ip_src.S_un.S_addr), sizeof(ULONG));
					*tcp_checksum = *tcp_checksum - 0x0101;
				...
				NdisRetreatNetBufferDataStart(ClonedNb, 14 + (int)size_iphl, FALSE, NULL);
				DEBUGP(DL_TRACE, "\n====address test====\n");
				DEBUGP(DL_TRACE, "original nbl : %p \n", CurrNbl);
				DEBUGP(DL_TRACE, "cloned nbl : %p \n", ClonedNbl);
				...
				DEBUGP(DL_TRACE, "\n===checksum===\n");
				DEBUGP(DL_TRACE, "TCPChecksumSucceeded : %d, IpCheckSumSucceeded : %d \n", ChecksumInfo2.Receive.TcpChecksumSucceeded, ChecksumInfo2.Receive.IpChecksumSucceeded);
				DEBUGP(DL_TRACE, "IP CHECKSUM : %x \n", m_ip->chksum);
		...
		NdisFIndicateReceiveNetBufferLists(pFilter->FilterHandle, FirstNbl, PortNumber, NumberOfNetBufferLists, ReceiveFlags);
